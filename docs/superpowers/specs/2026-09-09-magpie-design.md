...
- `docs/design/*.html` — the design screens (download control and version menu, logo candidates and the 16px decision, badge states, pill and toast, multi-provider and the capability matrix, popup v2, tool outputs). **Kept in the repository**: the specification references them and they are the visual evidence for its decisions; left in a temporary directory, the spec loses its own references
